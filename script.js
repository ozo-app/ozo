...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 📘 Câu mẫu → phát file mp3 thay vì speechSynthesis
  const exampleAudios = {
    "Hello, how are you?": "audio/hello_how_are_you.mp3",
    "I'm fine, thank you!": "audio/im_fine_thank_you.mp3",
    "What's your name?": "audio/whats_your_name.mp3"
  };

  document.querySelectorAll(".example").forEach((el) => {
    el.addEventListener("click", () => {
      const text = el.dataset.text;
      const src = exampleAudios[text];
      if (src) {
        const audio = new Audio(src);
        audio.play();
      } else {
        alert("Chưa có audio mẫu cho câu này.");
      }
    });
  });
